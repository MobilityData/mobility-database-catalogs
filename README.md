...
| is_seasonal | Enum | Optional | Indicates whether the feed is only available seasonally (e.g. summer or winter service). <ul><li>`False`: Feed is available year-round.</li><li>`True`: Feed is only available during a specific season.</li><li>`null`: There is not enough information to describe the seasonality of the feed.</li></ul> |
...
| - bounding_box  | Object|System generated             | Bounding box of the feed  when it was first added to the catalog. Contains `minimum_latitude`, `maximum_latitude`, `minimum_longitude`, `maximum_longitude` and `extracted_on` fields. If the bounding box information displays as "null", you can check any potential feed errors with [the GTFS validator](https://github.com/MobilityData/gtfs-validator). Note: it is not always updated in the catalogs JSON file, but is always accurate the Mobility Database API.  |
