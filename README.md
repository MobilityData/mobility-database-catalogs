...
| unstable_producer_url | Enum | Optional | Indicates whether the feed's producer URL is stable and unchanging over time. <ul><li>`False`: Producer URL is stable and unchanging over time.</li><li>`True`: Producer URL is unstable and changes over time. This may be because it contains a timestamp in the URL, or because it is tied to a GTFS vendor which the agency has changed from.</li><li>`null`: There is not enough information to describe the producer URL.</li></ul> |
