...
echo "Using GitHub token: ${GH_TOKEN:0:4}********"
...
# Reset local branch to remote HEAD (removes local commits)
...
EXISTING_PR=$(GH_TOKEN="${GH_TOKEN}" gh pr list --head "$BRANCH_NAME" --json number -q '.[0].number')

if [ -n "$EXISTING_PR" ]; then
  echo "Pull request already exists. Updating PR #$EXISTING_PR"
  PR_URL=$(GH_TOKEN="${GH_TOKEN}" gh pr view $EXISTING_PR --json url -q '.url')
else
  echo "Creating new pull request"
  PR_URL=$(GH_TOKEN="${GH_TOKEN}" gh pr create --title "Automated Pull Request — New/Updated feed(s) [SOURCES]" \
